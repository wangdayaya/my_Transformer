...
def main(ids, vocab):
    ...
    predict_input_fn = tf.estimator.inputs.numpy_input_fn( x={"enc_inputs": X}, num_epochs=1,  shuffle=False)
...
def _make_estimator():
    ...
    # Using CPU
    run_config = tf.contrib.learn.RunConfig(  model_dir=Config.train.model_dir, session_config=tf.ConfigProto( device_count={'GPU': 0}  ))

    model = Model()
    return tf.estimator.Estimator( model_fn=model.model_fn,  model_dir=Config.train.model_dir, params=params, config=run_config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src', type=str, default='example source sentence',  help='input source sentence')
