...
class Graph:
    ...
    def build(self, encoder_inputs = None, decoder_inputs = None):
        ...
        if self.mode == tf.estimator.ModeKeys.TRAIN:
            ...
            decoder_input_start_1 = tf.slice(next_decoder_inputs, [0,1], [self.batch_size, Config.data.max_seq_length-1])
            predictions = tf.concat([decoder_input_start_1, tf.zeros([self.batch_size, 1], dtype=tf.int32)], axis=1)
    ...
    def build_embed(self , inputs, encoder=True, reuse=False):
        with tf.variable_scope("Embeddings", reuse=reuse, dtype=self.dtype) as scope:
            embedding_encoder = tf.get_variable("embedding_encoder", [Config.data.source_vocab_size, Config.model.model_dim], self.dtype)
            embedding_decoder = tf.get_variable("embedding_decoder", [Config.data.target_vocab_size, Config.model.model_dim], self.dtype)
